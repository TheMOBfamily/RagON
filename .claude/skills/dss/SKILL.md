...
## Output Workflow (KAIZEN - INCREMENTAL)

**Ref:** `.fong/instructions/mindsets/mindset-agile-kaizen-think-big-do-baby-steps.md`

```
╔═══════════════════════════════════════════════════════════╗
║  ❌ OLD (BATCH): Query all → Write at end                 ║
║  ✅ NEW (KAIZEN): Skeleton → Query → Fill → Repeat        ║
╚═══════════════════════════════════════════════════════════╝

KAIZEN WORKFLOW:
1. SELECT PROTOCOL: DSS5 or DSS7
2. CREATE SKELETON FILE FIRST (empty sections)
3. QUERY SOURCE #1 → FILL section → SAVE
4. QUERY SOURCE #2 → FILL section → SAVE
5. ... repeat for all sources
6. CROSS-VERIFY → FILL conclusion
7. COMMIT after each major fill
```

**Why Kaizen:**
- Progress visible immediately
- Partial results saved (crash-safe)
- Query informs next query
- Fill-in reveals gaps → query more
...
| Pattern | Example |
|---------|---------|
| `DSS{5\|7}-{topic}-research-{YYYYMMDD-HHMMSS}.md` | `DSS5-chi-phi-phd-research-20260123-160743.md` |
| `DSS{5\|7}-{topic}-deep-research-prompt-{YYYYMMDD-HHMMSS}.md` | `DSS5-chi-phi-phd-deep-research-prompt-20260123-160743.md` |
| `DSS{5\|7}-{topic}-research-{YYYYMMDD-HHMMSS}.pdf` | `DSS5-chi-phi-phd-research-20260123-160743.pdf` |

**Output files trong folder:**

| File | Mục đích |
|------|----------|
| `DSS*-{topic}-research-{datetime}.md` | Research output với footnotes + diagrams |
| `DSS*-{topic}-research-{datetime}.pdf` | PDF export via Textor |
| `DSS*-{topic}-deep-research-prompt-{datetime}.md` | Prompt RCIFENI-O cho Deep Research tools |

---

## Diagram Requirements (MANDATORY)

**Rule:** Mỗi DSS output PHẢI có ít nhất 1 diagram minh họa.

**Priority (chọn 1+):**

| Type | Tool | When to Use |
|------|------|-------------|
| 1. PlantUML | `@startuml/@enduml` | Flowcharts, sequences, components |
| 2. BPMN | bpmn.io notation | Business processes |
| 3. Mermaid | ` ```mermaid ` | Quick flowcharts, mindmaps |
| 4. TikZ/LaTeX | (limited) | Mathematical diagrams only |

**References:**
- PlantUML: `.fong/instructions/textor-mdlatex/06-plantuml-guide.md`
- BPMN: `.fong/instructions/BPMN/bpmn-vi.md`
- Mermaid: `.fong/instructions/textor-mdlatex/03-mermaid-guide.md`

**PlantUML Skeleton (Blue-Gray CUD palette):**
```plantuml
@startuml dss-diagram
skinparam dpi 300
...
**Filename:** `DSS{5|7}-{topic}-research-{YYYYMMDD-HHMMSS}.md`

```markdown
---
title: "{Topic} Research"
subtitle: "Custom-DSS for Thanh-Phong Lam"
author: "Custom-DSS for Thanh-Phong Lam"
date: "{YYYY-MM-DD}"
protocol: "DSS5|DSS7"
sources_used: N
version: "1.0.0"
...
This document was generated using Custom-DSS (Decision Support System) methodology with multi-source verification. All information has been cross-checked from at least 3 independent sources. While every effort has been made to ensure accuracy, readers should verify critical information from primary sources before making decisions.

**Generated by:** Custom-DSS for Thanh-Phong Lam
**Protocol:** DSS{5|7} ({5|7} sources, {15|21} queries minimum)
**Date:** {YYYY-MM-DD HH:MM:SS} (+07)
...
```plantuml
@startuml research-overview
' [Diagram minh họa research flow]
@enduml
```
