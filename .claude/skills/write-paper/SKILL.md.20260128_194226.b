...
READ .fong/instructions/academic/scientific-plain-english-checklist.md
READ .fong/instructions/academic/instructions-write-abstract.json
READ .fong/instructions/instructions-micro-learning-newton-galileo-david-trafimow.json
...
## Research Ethics & Scientific Integrity (CRITICAL)

> ⚠️ **EXTREME MINDSET** — Đạo đức liêm chính khoa học là BẮT BUỘC. Vi phạm = Thất bại. Không ngoại lệ.

**Ref:** `instructions-micro-learning-newton-galileo-david-trafimow.json` (Trafimow Principle #10: Ethics)


### FFP Rule (Zero Tolerance)

| Code | Violation | Definition | Consequence |
|------|-----------|------------|-------------|
| **F** | Fabrication | Bịa đặt số liệu không tồn tại | ABORT paper |
| **F** | Falsification | Sửa đổi/bóp méo số liệu thật | ABORT paper |
| **P** | Plagiarism | Sao chép không trích dẫn | ABORT paper |

**Rule:** Phát hiện FFP = Dừng ngay. Không tiếp tục. Không ngoại lệ.


### Trafimow 10 Principles (MANDATORY)

> Source: David Trafimow Workshop 2026-01-14

| # | Principle | Application in Paper Writing |
|---|-----------|------------------------------|
| 1 | **Diversification** | Multiple research lines. Never all eggs in one basket. |
| 2 | **Obsession** | Deep focus on topic. Know EVERYTHING about it. |
| 3 | **Inefficiency** | Allow exploration time. Not all paths lead somewhere. |
| 4 | **Novelty** | Seek what's NEW. Reviewers want fresh ideas. |
| 5 | **Pathos/Drama** | Create emotional hook. Why should reader CARE? |
| 6 | **Clarity** | Crystal clear writing. No ambiguity. Zero jargon. |
| 7 | **Rhetoric** | Persuade with logic AND emotion. |
| 8 | **Persistence** | Reject → Revise → Resubmit. Never give up. |
| 9 | **Collaboration** | Find complementary strengths. Co-authorship. |
| 10 | **Ethics** | **Integrity above all. No shortcuts. No fabrication.** |

**Critical:** Principle #10 (Ethics) OVERRIDES all others. Ethical paper > Published paper.
...
|-------------|-------------|----------|
| **Data availability** | Raw data phải công khai hoặc giải thích lý do không công khai | DOI, repository link, or statement |
| **Code availability** | Scripts phải share (GitHub, Zenodo, OSF) | Repository URL |
| **Methodology detail** | Đủ chi tiết để người khác replicate | Step-by-step in Methods section |
| **Software versions** | Ghi rõ version của mọi tool/package | R 4.3.2, Python 3.11, etc. |
| **Random seeds** | Nếu có randomness, ghi seed | set.seed(12345) |
| **Environment** | OS, hardware nếu relevant | Linux Ubuntu 22.04, etc. |

**Rule:** Nếu không thể replicate = Không phải science.


### Data Integrity Checklist (KILL ITEMS)

> ⚠️ Abort paper nếu BẤT KỲ item nào FAIL.

- [ ] **Real data?** — Số liệu từ nguồn có thể verify (NOT fabricated)
- [ ] **Unmodified?** — Số liệu không bị sửa đổi để fit hypothesis (NOT falsified)
- [ ] **Original work?** — Tất cả text là original hoặc properly cited (NOT plagiarized)
- [ ] **Transparent methods?** — Methodology đủ chi tiết để replicate
- [ ] **Data accessible?** — Data có thể access hoặc có lý do hợp lệ không share
- [ ] **Conflicts declared?** — Conflict of interest được khai báo


### Anti-Patterns (FORBIDDEN)

| ❌ FORBIDDEN | Why | Consequence |
|--------------|-----|-------------|
| Bịa số liệu | Fabrication | Paper ABORT |
| Sửa outliers để fit model | Falsification | Paper ABORT |
| Copy-paste không cite | Plagiarism | Paper ABORT |
| P-hacking (try until p<0.05) | Cherry-picking | Reject results |
| HARKing (hypothesis after results) | Dishonest | Disclose in paper |
| Salami slicing không hợp lý | Self-plagiarism | Review ethics |
| Ghost/Gift authorship | Authorship fraud | Revise author list |

**Rule:** Phát hiện anti-pattern = STOP. Fix BEFORE continue.
...
1. BEFORE data collection:
   - Pre-register hypothesis (OSF, AsPredicted)
   - Define analysis plan
   - Declare conflicts

2. DURING analysis:
   - Use ONLY pre-specified methods
...
**Submission Checklist (21 items - Ethics Enhanced):**

**Ethics & Integrity (KILL ITEMS - abort if fail):**

- [ ] **FFP verified?** (No Fabrication, Falsification, Plagiarism)
- [ ] **Data real & unmodified?** (From verifiable source)
- [ ] **Reproducibility ensured?** (Code + Data + Methods available)
- [ ] **Pre-registration done?** (If applicable: OSF, AsPredicted)
- [ ] **All results reported?** (Including null/negative findings)
- [ ] **Author contributions clear?** (CRediT taxonomy)
...
# 5. Cautions:
- anti: **FABRICATION** (bịa số liệu) → ABORT PAPER
- anti: **FALSIFICATION** (sửa số liệu) → ABORT PAPER
- anti: **PLAGIARISM** (đạo văn) → ABORT PAPER
- anti: **P-hacking** (try until p<0.05) → REJECT RESULTS
- anti: **HARKing** (hypothesis after results) → DISCLOSE
...
- anti: Hide negative/null results
...
- [ ] **ETHICS: Real data?** (NOT fabricated)
- [ ] **ETHICS: Unmodified data?** (NOT falsified)
- [ ] **ETHICS: Original work?** (NOT plagiarized)
- [ ] **ETHICS: Reproducible?** (Methods + Data + Code available)
...
| **Academic Writing EN** | `.fong/instructions/academic/scientific-plain-english-checklist.md` |
| **Rephrase Academic VN** | `.fong/instructions/academic/rephrase-academic-vi.sh` |
| **Abstract Writing** | `.fong/instructions/academic/instructions-write-abstract.json` |
| **Micro-Learning Framework** | `.fong/instructions/instructions-micro-learning-newton-galileo-david-trafimow.json` |
...
[6] scientific-plain-english-checklist.md
[7] rephrase-academic-vi.sh
[8] instructions-write-abstract.json
[9] instructions-micro-learning-newton-galileo-david-trafimow.json
