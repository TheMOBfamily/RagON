...
**Logic auto-start:**
1. Kiểm tra service có đang chạy không (`curl http://localhost:2011`)
...
```bash
curl -X DELETE http://localhost:2011/cache//home/fong/Projects/mini-rag/example
```
...
```bash
curl -X DELETE http://localhost:2011/cache
```
...
curl -X DELETE http://localhost:2011/cache/path/to/unused/pdfs
```
...
```bash
# Query
curl -X POST http://localhost:2011/query \
