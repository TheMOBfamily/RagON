...
> Knowledge is DNA. Retrieve first. Reuse. Evolve. Store. Repeat.

---

## Khi Nhận {memetic} Trong Task

**Nghĩa là gì?**
Đáp án có sẵn. Không cần sáng tạo. Tìm là ra.

**Ở đâu?**
`.memory/` → `{workspace}` → `.fong/instructions/` → codebase → DKM.

**Bằng gì?**
`smart-search` và `rg`. By all costs.

**Workflow:**
```
1. Gặp trở ngại → ĐỪNG đoán
2. smart-search keyword → tìm trong .memory/
3. Không có → rg trong codebase
4. Không có → query DKM
5. Tìm ra → ÁP DỤNG NGAY
6. Giải quyết xong → GHI .memory/ cho session sau
```

**Anti-pattern:**
- ❌ Tự suy luận khi chưa search
- ❌ Hỏi user khi đáp án có sẵn
- ❌ Giải xong nhưng không ghi lại

**Mantra:**
> Mọi bài học đã có sẵn. Chỉ cần tìm. Tìm xong thì ghi.
