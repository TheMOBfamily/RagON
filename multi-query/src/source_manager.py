...
def discover_sources(base_dir: str, external_sources: List[str] = None, no_sort: bool = False) -> List[SourceInfo]:
    """
    ...
    Args:
        ...
        no_sort: If True, preserve discovery order (no sorting)
    ...
    # Sort by name unless --no-sort is specified
    if no_sort:
        return sources
    else:
...
def list_sources(base_dir: str, external_sources: List[str] = None, no_sort: bool = False) -> List[SourceInfo]:
    """List available sources with numbered display"""
    sources = discover_sources(base_dir, external_sources, no_sort=no_sort)
...
def list_pdfs_metadata(base_dir: str, no_sort: bool = False) -> dict[str, str]:
    """
    List all PDFs from hash-based structure with metadata.

    OPTIMIZED: Returns dict mapping hash->filename (23.87% token savings).

    Args:
        base_dir: Base directory (DKM-PDFs)
        no_sort: If True, preserve discovery order (no randomization)

    Returns:
        Dict mapping file_hash to filename
        Example: {"abc123...": "book.pdf", "def456...": "book2.pdf"}
    """
    base_path = Path(base_dir)
    pdfs = {}
    ...
    # Randomize the order unless --no-sort is specified
    if not no_sort:
