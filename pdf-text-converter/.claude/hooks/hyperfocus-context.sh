...
# Version: 1.6.0 - Auto-fallback with debug logging
# Updated: 2025-11-13
...
if [ -f "$PROJECT_ROOT/.claude/hooks/init-prompt.md" ]; then
  log_debug "Found init-prompt.md, printing content (compact mode)"
  echo ""
  echo "--- INIT PROMPT INSTRUCTIONS ---"
  cat "$PROJECT_ROOT/.claude/hooks/init-prompt.md" | sed '/^$/d' | sed 's/  */ /g'
  echo ""
  echo "--- END INIT PROMPT ---"
else
  log_debug "WARNING: init-prompt.md not found at $PROJECT_ROOT/.claude/hooks/init-prompt.md"
fi
